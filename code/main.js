...
function getReactivationOptions(policy) {
  return [
    new ReactivationOption({
      type: 'reinstatement',
      settlementAmount: policy.balance,
...
async function getClaimAssignmentOptions(policy, claim, currentUser, stage) {
  const userSets = {
    capture: [
      {
        ...
        role: 'Admin',
        description: 'Available for claim capture',
      },
      {
      ...
      },
      {
        ...
        description: 'Available for claim capture support',
      },
    ],
    review: [
      {
        ...
        email: 'dinesh@piedpiper.com',
        role: 'Claims Assessor',
        description: 'Available for claim review',
      },
      {
        id: '00000000-0000-0000-0000-000000000011',
        name: 'Payment Manager',
        email: 'payment_manager@piedpiper.com',
    ...
    acknowledge: [
      {
        ...
        email: 'call_center_agent@piedpiper.com',
        role: 'Call Center Agent',
        description: 'Handles claim approvals and final acknowledgments',
      },
      {
        id: '00000000-0000-0000-0000-000000000001',
        name: 'Erlich Bachman',
        email: 'erlich@aviato.com',
        role: 'Admin',
        description: 'Available for final acknowledgment',
      },
    ],
  };

  const availableUsers = userSets[stage] || userSets.review;
  let assignedToUser = availableUsers[0];

  if (currentUser && currentUser.user_id) {
    const currentUserInList = availableUsers.find((user) => user.id === currentUser.user_id);
    if (currentUserInList) {
      assignedToUser = {
        id: currentUser.user_id,
        name: `${currentUser.first_name} ${currentUser.last_name}`,
        email: currentUser.email,
        role: currentUser.organization_role_name || 'Claims User',
        description: `Current user - available for ${stage}`,
      };

      const userIndex = availableUsers.findIndex((user) => user.id === currentUser.user_id);
      if (userIndex !== -1) {
        availableUsers[userIndex] = assignedToUser;
      }
    }
  }
...
function validateAlterationPackageRequest({ alteration_hook_key, data, policy, policyholder }) {
  if (alteration_hook_key === 'hook_1') {
    if (policy && policyholder) {
      console.log(`Policy ID: ${policy.policy_id};`, `Policyholder ID: ${policyholder.policyholder_id}`);
    }
    return Joi.validate(
      data,
      Joi.object().keys({
...
const genderFromID = (id) => {
  const code = parseInt(id.toString().slice(6, 10), 10);
...
function getQuote(data) {
  console.log('Executing getQuote');
...
function getPolicy(application, policyholder) {
  return new Policy({
    ...
    module: { ...application.module, instructions: [{ type: 'test' }] },
    charges: [
      {
        type: 'fixed',
        name: 'Fixed Fee',
...
function customScheduledFunction() {
  console.log("I'm a scheduled function, and I'm running now!");
  return [
    {
...
const mainRootAPIRequest = async (uri, method, body = null) => {
  const options = {
    method,
    headers: {
  ...
  try {
    ...
    const response = await fetch(`${process.env.API_BASE_URL}/v1/insurance/${uri}`, options);
    console.log(options);
    console.log('BODY___ ', options.body);

    // Store the response data to avoid consuming the body twice
    const responseData = await response.json();
    console.log('Response___', JSON.stringify(responseData));
...
const processInstructions = async ({ policy }) => {
  const newModule = {
    ...policy.module,
    has_process_instructions_executed: true,
  };
  return [
    {
      name: 'update_policy',
...
const patchMultipleBlockStates = async ({ claimId, body }) => {
  // Make a single API request to update all block states
  const response = await mainRootAPIRequest(`claims/${claimId}/blocks`, 'PATCH', body);

  return response;
};

const afterClaimBlockUpdated = async ({ block_key, policy, policyholder, claim }) => {
  if (block_key === 'payout_details_account_type_dropdown') {
    await accountTypeBlockChange({ claim, policy, policyholder });
  }

  const newModule = {
    ...policy.module,
    has_claim_block_updated: true,
    claim_block_updated_key: block_key,
  };
  return [
    {
      name: 'update_policy',
...
const accountTypeBlockChange = async ({ claim, policy, policyholder }) => {
  const account_type = claim.block_states['payout_details_account_type_dropdown'].option_key;

  if (account_type === 'cheque_account') {
    const blockStatesToReset = [
      {
        key: 'test_background_update',
        block_state: {
          value: 'cheque selected',
          type: 'input.text',
        },
      },
      {
        key: 'test_background_date',
        block_state: {
          value: '2020-01-01',
          type: 'input.date',
        },
      ...
      {
        key: 'test_background_paragraph',
        block_state: {
          value: 'Cheque account selected',
          type: 'input.paragraph',
        },
      },
      {
        key: 'test_background_currency',
        block_state: {
          value: 100,
          type: 'input.currency',
        },
      ...
      {
        key: 'test_background_dropdown',
        block_state: {
          option_value: 'Cheque account selected',
          option_key: 'cheque_account',
          type: 'dropdown',
        },
      },
      {
        key: 'test_background_radio',
        block_state: {
          option_value: 'Cheque account selected',
          option_key: 'cheque_account',
          type: 'radio',
    ...
    console.log(JSON.stringify(blockStatesToReset, null, 2));

    await patchMultipleBlockStates({
      claimId: claim.claim_id,
      body: blockStatesToReset,
    });
  }
  ...
  if (account_type === 'savings_account') {
    const blockStatesToReset = [
      ...
      },
      {
        key: 'test_background_paragraph',
        block_state: {
          value: 'Savings account selected',
          type: 'input.paragraph',
        },
      },
      {
      ...
      },
      {
        key: 'test_background_dropdown',
        block_state: {
          option_value: 'Savings account selected',
          option_key: 'savings_account',
          type: 'dropdown',
        },
      },
      {
...
const beforeClaimSentToReview = async ({ policy, claim }) => {
  const newModule = {
    ...policy.module,
    before_claim_sent_to_review_has_run: true,
    before_claim_sent_to_review_has_run_claim_id: claim.claim_id,
  };
  return [
    {
      name: 'update_policy',
...
const afterClaimSentToReview = async ({ policy, claim }) => {
  const newModule = {
    ...policy.module,
    after_claim_sent_to_review_has_run: true,
    after_claim_sent_to_review_has_run_claim_id: claim.claim_id,
  };
  return [
    {
      name: 'update_policy',
    ...
    },
    {
      ...
      data: {
        ...newModule,
        claim_sent_to_review: true,
        claim_sent_to_review_id: claim.claim_id,
      },
    },
  ];
};
...
const afterClaimDecisionAcknowledged = ({ claim, policy, policyholder }) => {
  const newModule = {
    ...policy.module,
    claim_status: claim.claim_status,
    claim_approval_status: claim.approval_status,
    policyholder_id_number: policyholder.id.number,
    has_after_claim_decision_acknowledged_hook_run: true,
  };
  return [
    {
      name: 'update_policy',
...
const afterClaimDecision = async ({ claim, policy, policyholder }) => {
  const newModule = {
    ...policy.module,
    after_claim_decision_has_run: true,
    after_claim_decision_has_run_claim_id: claim.claim_id,
  };
  return [
    {
      name: 'update_policy',
...
async function afterClaimClosed({ policy, claim, policyholder }) {
  const newModule = {
    ...policy.module,
    claim_status: claim.claim_status,
    after_claim_closed_has_run: true,
    after_claim_closed_has_run_claim_id: claim.claim_id,
  };
  return [
    {
      name: 'update_policy',
    ...
    },
    {
      ...
      data: {
        ...newModule,
        claim_closed: true,
        claim_closed_id: claim.claim_id,
      },
    },
  ];
}
...
function afterPolicyExpired({ policy, policyholder }) {
  return [
    {
      name: 'update_policy',
      data: {
        module: {
          ...policy.module,
          has_after_policy_expired_hook_run_policyholder_id: policyholder.policyholder_id,
          has_after_policy_expired_hook_run: true,
        },
      },
    },
  ];
...
function afterPolicyActivated({ policy, policyholder }) {
  return [
    {
      name: 'update_policy',
      data: {
        module: {
          ...policy.module,
          has_after_policy_activated_hook_run_policyholder_id: policyholder.policyholder_id,
          has_after_policy_activated_hook_run: true,
        },
      },
    },
  ];
...
function afterPolicyPaymentMethodAssigned({ policy, policyholder }) {
  return [
    {
      name: 'update_policy',
      data: {
        module: {
          ...policy.module,
          has_after_policy_payment_method_assigned_hook_run_policyholder_id: policyholder.policyholder_id,
          has_after_policy_payment_method_assigned_hook_run: true,
        },
      },
    },
  ];
...
const afterAlterationPackageApplied = ({ policy, policyholder, alteration_package, alteration_hook_key }) => {
  const module = {
    ...policy.module,
    after_alteration_package_applied_hook: `Alteration package ID: ${alteration_package.alteration_package_id}. Alteration hook key: ${alteration_hook_key}.`,
  };
  return [
    {
      name: 'update_policy',
...
const quoteRequestSchema = Joi.object()
  .keys({
    cover_amount: Joi.number()
      ...
      .min(5000 * 100)
      .max(100000 * 100)
      .meta({ currency: 'ZAR', unit: 'cents', displayUnit: 'rand', divisor: 100 })
      .required(),
    // Main policyholder
    age: Joi.number().integer().positive().min(18).max(70).required(),
    gender: Joi.valid(['male', 'female']).allow(null).optional(),
...
function describeQuoteRequestSchema() {
  return quoteRequestSchema.describe();
}

function validateQuoteRequest(data) {
  console.log('Executing validateQuoteRequest');
  return Joi.validate(data, quoteRequestSchema);
}

const applicationRequestSchema = Joi.object({
  spouse: Joi.when('$spouse_included', {
    is: true,
    then: Joi.object({
      first_name: Joi.string().required(),
    ...
    otherwise: Joi.any().strip(), // or Joi.valid(null)
  }),
  ...
  user_group_id: Joi.string().uuid().allow(null).optional(),
...
function getPolicyModuleDisplayData(policy, policyholder) {
  const currentAge = policyholder.dateOfBirth ? ageFromDateOfBirth(policy.policyholder.dateOfBirth) : policy.module.age;
  const shouldThrowError = policyholder.first_name === 'Error' && policyholder.last_name === 'Error';

  if (shouldThrowError) {
    throw new Error('Error getting policy module display data');
  }
...
function describeApplicationRequestSchema() {
  return applicationRequestSchema.describe();
}

function validateApplicationRequest(data, policyholder, quotePackage) {
  const flags = quotePackage.module; // {spouse_included:…, children_included: …}
  return Joi.validate(data, applicationRequestSchema, { context: flags });
}
