...
/* global moment ReactivationOption InvalidRequestError AlteredPolicy AlterationPackage orgTimezone QuotePackage Application Policy generatePolicyNumber Joi RequotePolicy root */
...
function validateAlterationPackageRequest({ alteration_hook_key, data }) {
  if (alteration_hook_key === 'hook_1') {
    console.log('running hook - sandbox');
    return Joi.validate(
      data,
      Joi.object().keys({
...
function getQuote(data) {
  ...
  const apiUrl = new URL('/api/endpoint', 'https://example.com');
  const searchParams = new URLSearchParams('key=value&other=data');

  // URL is a global, just like console, process, Buffer, etc.
  ...
  return new QuotePackage({
    ...
    module: {
      type: 'root_funeral',
      embed_session_id: data.embed_session_id,
      cover_amount: coverAmount,
      age,
      gender,
      spouse_included: spouseIncluded,
...
const quoteRequestSchema = Joi.object()
  .keys({
    cover_amount: Joi.number()
      ...
      .required(),
    // Main policyholder
    age: Joi.number().integer().positive().min(18).max(70).required().description('Age of the main member'),
    gender: Joi.when('spouse_included', {
      is: true,
      then: Joi.when('spouse.gender', {
        is: Joi.exist().not(null),
        then: Joi.valid(['male', 'female']).required(),
        otherwise: Joi.valid(['male', 'female']).allow(null).optional(),
      }),
      otherwise: Joi.valid(['male', 'female']).allow(null).optional(),
    }),
    // Spouse
...
async function getClaimAssignmentOptions(policy, claim, currentUser, stage) {
  ...
  if (Array.isArray(parsedData)) {
    ...
    allUsers = parsedData.users;
  } else if (parsedData && Array.isArray(parsedData.data)) {
    allUsers = parsedData.data;
  } else {
  ...
  const stageRoleMappings = {
    capture: [
      'Admin',
      'Claims assessor',
      'Call center agent',
      'Call center agent unrestricted',
      'Call center agent restricted',
      'Claims no assignment',
    ],
    review: [
      'Claims assessor',
      'Payment manager',
      ...
      'Claims supervisor',
      'Finance manager',
      'Claims approval supervisor',
      'Claims goodwill supervisor',
      'Claims repudiation supervisor',
      'Claims no claim supervisor',
      'Claims no assignment',
    ],
    acknowledge: [
      'Call center agent',
  ...
  // Define descriptions for each stage
  const stageDescriptions = {
    capture: {
      Admin: 'Available for claim capture',
      'Claims assessor': 'Available for claim capture',
      'Call center agent': 'Available for claim capture',
      'Call center agent unrestricted': 'Available for claim capture',
      'Call center agent restricted': 'Available for claim capture support',
      'Claims no assignment': 'Available for claim capture',
    },
    review: {
      'Claims assessor': 'Available for claim review',
      'Payment manager': 'Available for claim review',
      Admin: 'Available for claim review',
      'Claims supervisor': 'Available for claim review and supervision',
      'Finance manager': 'Available for financial review',
      'Claims approval supervisor': 'Available for approval review',
      'Claims goodwill supervisor': 'Available for goodwill review',
      'Claims repudiation supervisor': 'Available for repudiation review',
      'Claims no claim supervisor': 'Available for no claim review',
      'Claims no assignment': 'Available for claim review',
    },
    acknowledge: {
      'Call center agent': 'Handles claim approvals and final acknowledgments',
      'Call center agent unrestricted': 'Handles claim approvals and final acknowledgments',
      Admin: 'Available for final acknowledgment',
      'Claims supervisor': 'Available for supervisory acknowledgment',
      'Claims assessor': 'Available for assessment acknowledgment',
      'Claims no assignment': 'Available for final acknowledgment',
    },
  };

  // Filter users based on stage
  const allowedRoles = stageRoleMappings[stage] || stageRoleMappings.review;
  const filteredUsers = allUsers.filter(
    (user) => user.organization_role_name && allowedRoles.includes(user.organization_role_name),
  );
  ...
  const availableUsers = filteredUsers.map((user) => ({
    ...
    email: user.email,
    role: user.organization_role_name,
    description:
      (stageDescriptions[stage] && stageDescriptions[stage][user.organization_role_name]) || `Available for ${stage}`,
  }));

  // Default to first available user or fallback
  let assignedToUser = availableUsers[0];

  // Check if current user should be assigned
  if (currentUser && currentUser.user_id) {
    const currentUserInList = availableUsers.find((user) => user.id === currentUser.user_id);
    if (currentUserInList) {
      console.log('Current user found in available users, assigning them');
      assignedToUser = {
      ...
      console.log('Current user not found in available users');
    }
  }
  ...
  spouse: Joi.when('$spouse_included', {
    ...
    then: Joi.object({
      ...
      date_of_birth: Joi.date()
        ...
        .max('now')
        .required()
        .description('Date of birth for spouse in the YYYY-MM-DD format'),
    }).required(),
      ...
      .items(
        Joi.object({
          ...
          date_of_birth: Joi.date()
            ...
            .max('now')
            .required()
            .description('Date of birth for child in the YYYY-MM-DD format'),
        }).required(),
      )
      .required(),
    otherwise: Joi.any().strip(),
      ...
      .items(
        Joi.object({
          ...
          date_of_birth: Joi.date()
            ...
            .max('now')
            .required()
            .description('Date of birth for extended family member in the YYYY-MM-DD format'),
...
function validateApplicationRequest(data, policyholder, quotePackage) {
  const flags = quotePackage.module;
